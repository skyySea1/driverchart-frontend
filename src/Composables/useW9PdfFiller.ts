...
export function useW9PdfFiller() {
  ...
  async function fillW9Pdf(driver: Driver): Promise<Blob> {
    try {
      ...
      try {
        ...
        fillTextField(
          form,
          'topmostSubform[0].Page1[0].City[0]',
      ...
      // Conversion: Ensure the underlying data is backed by a standard ArrayBuffer
      // instead of a potential SharedArrayBuffer. Some environments or libraries may
      // use SharedArrayBuffer, which is not always accepted as a BlobPart and can cause
      // type errors when constructing Blob. We create a fresh ArrayBuffer and copy data.
   ...
   */
  function formatSSN(ssnNumber: string): string {
    const digits = ssnNumber.replace(/\D/g, '')
    if (digits.length === 9) {
