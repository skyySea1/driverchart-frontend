...
export const DriverFormSchema = DriverSchema.extend({
  ...
  dob: z.string().min(1, 'Date of birth is required').pipe(pastIsoDate('Date of birth must be in the past')),
  ...
  cdl: ComplianceItemSchema.extend({
    ...
    expiryDate: z
      .string()
      .min(1, 'CDL Expiration is required'),
  ...
  medical: ComplianceItemSchema.extend({
    expiryDate: z
      .string()
      .min(1, 'Medical Expiration is required'),
  ...
  mvr: ComplianceItemSchema.extend({
    expiryDate: z
      .string()
      .min(1, 'MVR Date is required')
      }),

  drugAlcohol: ComplianceItemSchema.extend({
    expiryDate: z
      .string()
      .min(1, 'Drug test date is required')
  ...
  emergencyContact: z.object({
    ...
    phone: z.string().min(1, 'Emergency contact phone is required').pipe(usPhoneNumber('Emergency contact phone is required')),
