  ...
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" role="dialog" aria-modal="true">
    ...
    <div class="relative w-full max-w-lg bg-white rounded-xl shadow-2xl flex flex-col max-h-[90vh]">
      ...
      <div class="px-6 py-5 overflow-y-auto space-y-6">
        <!-- System Warnings -->
        <div v-if="systemWarnings.length > 0" class="bg-amber-50 border border-amber-200 rounded-lg p-4 space-y-2">
        ...
        <!-- Manual Checklist -->
        <div class="space-y-3">
          <h4 class="text-sm font-bold text-slate-700 uppercase tracking-wide">Pre-Hire Checklist (Required)</h4>
           <div class="bg-slate-50 rounded-lg p-4 border border-slate-200">
             <div class="space-y-3">
               <label v-for="(item, idx) in checklistItems" :key="idx" class="flex items-start gap-3 cursor-pointer group">
                 ...
                 <span class="text-sm text-slate-600 peer-checked:text-slate-900 transition-colors group-hover:text-slate-800">
                   {{ item.label }}
                 </span>
               </label>
             </div>
        ...
        <div class="pt-2">
          <label class="flex items-center gap-3 cursor-pointer">
            ...
            <span class="text-sm font-medium text-slate-700">
              I confirm that <strong>{{ driverName }}</strong> has met all qualification standards and is cleared for duty.
            </span>
          </label>
        </div>
      </div>
      ...
      <!-- Footer -->
      <div class="px-6 py-4 border-t border-slate-100 flex items-center justify-end gap-3 shrink-0 bg-slate-50/50 rounded-b-xl">
        <BaseButton
          variant="secondary"
          label="Cancel"
          @click="$emit('close')"
...
const systemWarnings = computed(() => {
  const w: string[] = []
  if (!props.driver.license?.expiryDate || dayjs(props.driver.license.expiryDate).isBefore(dayjs())) {
    w.push('Driver License is expired or missing.')
  }
  if (!props.driver.medical?.expiryDate || dayjs(props.driver.medical.expiryDate).isBefore(dayjs())) {
