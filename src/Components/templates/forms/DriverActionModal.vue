      ...
      <div class="min-h-50">
        ...
        <div v-if="mode === 'flag_driver'" class="space-y-4">
          <p class="text-slate-600">
            You are about to flag this driver, provide the description of flag.
          </p>
          <textarea
            v-model="form.reason"
        ...
        <div v-if="mode.startsWith('request_')" class="space-y-6">
          <p class="text-slate-600">
            Please enter the email address(es) to receive the manual {{ requestTypeLabel }} upload
            link.
          </p>
          <div class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-4">
              ...
              <div class="md:col-span-2">
                ...
                <span class="text-[10px] text-slate-400 mt-1 block"
                  >Separate multiples with a comma.</span
                >
              </div>
            </div>
            ...
            <div class="space-y-2">
              <label class="text-sm font-medium text-slate-700"
                >Custom Email Message (Optional)</label
              >
              <textarea
        ...
        >
          ...
          <div class="md:col-span-2 space-y-4">
            <h4 class="font-bold text-slate-800">
              {{ mode === 'send_memo' ? 'Memo List' : 'Safety Policies' }}
            </h4>
            <p class="text-xs text-slate-500">
              Select one or more items below that you would like to send to your driver(s).
            </p>

            <div class="border border-slate-200 rounded-lg overflow-hidden">
              <div v-if="isLoadingMemos" class="p-8 text-center text-slate-400 text-xs">
                Loading items...
              </div>
              <table v-else class="w-full text-sm">
                <thead class="bg-slate-50 border-b border-slate-200">
                  <tr>
                    ...
                    <th class="p-3 text-left text-xs font-semibold text-slate-500 uppercase">
                      {{ mode === 'send_memo' ? 'Memo File' : 'Policy File' }}
                    </th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                  <tr
                    v-for="file in filteredMemos"
                    :key="file.id"
                  ...
                  <tr v-if="filteredMemos.length === 0">
                    <td colspan="2" class="p-8 text-center text-slate-400 text-xs italic">
                      No entries found. Upload one to start.
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          <div class="bg-slate-50 p-4 rounded-xl border border-slate-200 h-fit space-y-4">
            ...
            >
              ...
              <div class="text-[11px] text-slate-500">
                {{ isUploadingMemo ? 'Uploading...' : 'Select file(s)...' }}
              </div>
            </div>
            <div
              v-if="uploadStatus"
              class="text-[10px] text-center"
              :class="uploadStatus.type === 'success' ? 'text-green-600' : 'text-red-600'"
            >
...
const allSelected = computed(() => {
  return (
    filteredMemos.value.length > 0 && form.value.selectedFiles.length === filteredMemos.value.length
  )
})
...
const title = computed(() => {
  switch (props.mode) {
    ...
    case 'request_license':
      return "Send Driver's License Upload Request"
    case 'request_medical':
      return 'Send Medical Card Upload Request'
    case 'send_memo':
      return 'Memo Center'
    case 'send_policy':
      return 'Safety Policy Center'
    default:
      return 'Action'
  }
})
