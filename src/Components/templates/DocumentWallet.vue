<template>
  <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <button
            ...
            class="w-full py-1.5 text-[9px] uppercase tracking-tighter bg-slate-50 rounded-lg text-slate-600 hover:bg-slate-900 hover:text-white transition-all duration-300 cursor-pointer border border-slate-100 font-black"
          >
    ...
    <DocumentHistoryRegistry :logs="filteredLogs" @view-log="$emit('view-log', $event)" />
  </div>
</template>
...
const walletDocuments = computed(() => {
  ...
  const getFile = (field?: ComplianceItem | string | null) => {
  ...
  const getExpiry = (field?: ComplianceItem | null, fallback?: string) => {
  ...
  const docs = [
    {
      ...
      file:
        getFile(d.license) ||
        (typeof (d as Record<string, unknown>).licenseFront === 'string'
          ? ((d as Record<string, unknown>).licenseFront as string)
          : null),
      expiryDate: getExpiry(
        d.license,
        ((d as Record<string, unknown>).personalInfo as Record<string, string>)
    ...
    },
    {
      ...
      file:
        getFile(d.medical) ||
        (typeof (d as Record<string, unknown>).medicalCard === 'string'
          ? ((d as Record<string, unknown>).medicalCard as string)
          : null),
      expiryDate: getExpiry(
        d.medical,
        ((d as Record<string, unknown>).personalInfo as Record<string, string>)
    ...
    },
    {
      ...
      cfr: 'FMCSA 49 CFR Part 391.21',
      file:
        ((d as Record<string, unknown>).applicationFile as string) ||
        (d.appliedDate || d.createdAt ? 'APP_EXISTS' : null),
    },
  ]
  return docs
})
