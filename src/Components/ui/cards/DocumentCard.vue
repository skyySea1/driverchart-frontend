<template>
  <div class="group relative bg-white border border-slate-200 rounded-2xl p-4 shadow-sm hover:shadow-md hover:border-indigo-300 transition-all duration-300 flex flex-col justify-between">
    <!-- Header -->
    <div class="flex justify-between items-start mb-4">
      <div class="flex items-center gap-3">
         <div :class="['w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border transition-all', doc.file ? 'bg-indigo-50 text-indigo-600 border-indigo-100 shadow-sm' : 'bg-slate-50 text-slate-300 border-slate-100']">
           <component :is="icon" class="w-5 h-5" />
         </div>
         <div>
           <h3 class="font-black text-slate-900 text-xs uppercase tracking-tight">{{ doc.label }}</h3>
           <p class="text-[10px] font-bold text-slate-400 uppercase mt-0.5">{{ doc.cfr || 'FMCSA Requirement' }}</p>
         </div>
      </div>
    ...
    <!-- Content -->
    <div class="space-y-3">
      <div v-if="doc.expiryDate" class="bg-slate-50 rounded-lg p-2 flex justify-between items-center text-[10px] font-black uppercase tracking-tighter">
      ...
      <div class="flex gap-2">
        ...
        <button
          ...
          :class="['flex-1 text-center py-2 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all flex items-center justify-center gap-1 shadow-sm', doc.file ? 'bg-slate-900 text-white hover:bg-slate-800' : 'bg-indigo-600 text-white hover:bg-indigo-700']"
        >
