<template>
  <BaseModal
    :isOpen="isOpen"
    :title="doc?.label || 'Signature Proof'"
    size="max-w-2xl"
    @close="$emit('close')"
  >
    <div class="space-y-6">
      <div
        class="bg-slate-50 border border-slate-200 rounded-2xl p-8 space-y-8 relative overflow-hidden"
      >
        <!-- Watermark -->
        ...
        <div class="text-center space-y-2 relative z-10">
          <div
            class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-indigo-600 text-white shadow-xl shadow-indigo-200 mb-4"
          >
        ...
        <div class="space-y-6 relative z-10">
          <div class="grid grid-cols-2 gap-8">
            <div class="space-y-1">
              <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest"
                >Signer Name</span
              >
              <p class="text-lg font-bold text-slate-900 border-b-2 border-slate-200 pb-1">
                {{ doc?.signature }}
              </p>
            </div>
            <div class="space-y-1">
          ...
          <div class="space-y-1">
            ...
            >
            <p
              class="text-xs font-mono font-bold text-indigo-600 bg-indigo-50 p-2 rounded-lg border border-indigo-100 flex items-center justify-between"
            >
              {{ verificationId }}
              <span
                class="bg-indigo-600 text-white px-2 py-0.5 rounded text-[8px] uppercase tracking-widest"
                >Secured</span
              >
            </p>
          </div>
        ...
        <div class="pt-8 flex flex-col items-center justify-center space-y-4">
          ...
          <div class="text-center space-y-1">
            <p class="text-[10px] font-serif italic text-slate-400">
              Digitally signed and timestamped by the applicant.
            </p>
            <p class="text-[9px] font-black text-slate-300 uppercase tracking-widest">
              Phoenix Bus DOT Compliance System
            </p>
          </div>
        </div>
      </div>
