<template>
  <div class="space-y-8">
    <div class="flex justify-between items-center print:hidden">
      ...
      <BaseButton
        label="Print Compliance Package"
        @click="printReports"
        :icon="Printer"
    ...
    <div class="hidden print:block mb-8 border-b-2 border-slate-900 pb-4">
      <div class="flex justify-between items-start">
        <div>
          ...
          <p class="text-slate-600 mt-1 font-bold uppercase text-sm">
            US DOT #1234567 | Charter Bus Operations
          </p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 print:block print:space-y-8">
      <!-- Expiration Forecast -->
      <div
        class="bg-white p-6 rounded-xl shadow-sm border border-slate-100 print:shadow-none print:border-slate-200"
      >
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="text-left text-slate-500 border-b border-slate-200">
                ...
                <th class="pb-2 font-bold uppercase tracking-wider text-[10px] text-right">
                  Status
                </th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-50">
              <tr v-for="(item, i) in expiringSoon" :key="i">
                <td class="py-2 font-medium">{{ capitalizeName(item.driver) }}</td>
                <td class="py-2">
                  <span
                    class="px-2 py-0.5 bg-slate-100 text-slate-700 rounded text-[10px] font-bold uppercase print:bg-white print:border print:border-slate-200"
                  >
                    {{ item.type }}
                  </span>
                </td>
      ...
      >
        ...
        <div class="overflow-y-auto max-h-96 print:max-h-none">
          <div
            ...
            class="flex flex-col items-center justify-center py-8 text-green-600"
          >
            ...
            >
              <div class="flex justify-between items-center mb-2">
                ...
                <span class="text-[10px] font-bold text-red-600 uppercase"
                  >Missing Action Required</span
                >
              </div>
              <!--  todo review criteria field file x expirydate -->
              <div class="flex flex-wrap gap-2">
                <span
                  v-if="!d.cdl?.expiryDate"
                  class="text-[10px] bg-red-600 text-white px-2 py-0.5 rounded font-black uppercase"
                  >Cdl</span
                >
                <span
                  v-if="!d.medical?.expiryDate"
                  class="text-[10px] bg-red-600 text-white px-2 py-0.5 rounded font-black uppercase"
                  >Med Card</span
                >
                <span
                  v-if="!d.mvr?.expiryDate"
                  class="text-[10px] bg-red-600 text-white px-2 py-0.5 rounded font-black uppercase"
                  >Annual MVR</span
                >
                <span
                  v-if="!d.roadTest?.date"
                  class="text-[10px] bg-red-600 text-white px-2 py-0.5 rounded font-black uppercase"
                  >Road Test</span
                >
              </div>
    ...
    <!-- Compliance Matrix -->
    <div
      class="bg-white p-6 rounded-xl shadow-sm border border-slate-100 print:shadow-none print:border-slate-200 print:mt-8"
    >
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-center text-sm border-collapse">
          <thead>
            <tr
              class="bg-slate-50 text-slate-600 print:bg-white print:border-b-2 print:border-slate-900"
            >
              <th class="p-3 text-left font-bold uppercase tracking-wider text-[10px]">
                Driver Name
              </th>
    ...
    <!-- Print-only Footer -->
    <div
      class="hidden print:block mt-auto pt-8 border-t border-slate-200 text-center text-[10px] text-slate-400"
    >
...
const missingDocs = computed(() =>
  drivers.value.filter(
    (d) => !d.cdl?.expiryDate || !d.medical?.expiryDate || !d.mvr?.expiryDate || !d.roadTest?.date,
  ),
)
