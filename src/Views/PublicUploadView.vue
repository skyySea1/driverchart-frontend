  ...
  <div class="min-h-screen bg-slate-50 flex items-center justify-center p-4">
    <div class="w-full max-w-lg bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-200">
      ...
      <div v-if="isLoading" class="p-12 text-center space-y-4">
        <div class="w-8 h-8 rounded-full border-2 border-slate-200 border-t-indigo-600 animate-spin mx-auto"></div>
      ...
      <div v-else-if="error" class="p-12 text-center space-y-6">
        <div class="w-16 h-16 rounded-2xl bg-red-50 text-red-500 flex items-center justify-center mx-auto border border-red-100">
        ...
        <div class="pt-4 border-t border-slate-100 w-full">
           <p class="text-[10px] text-slate-400 font-medium">Please contact your dispatcher for a new link.</p>
        </div>
      </div>
      ...
      <div v-else class="animate-in fade-in slide-in-from-bottom-4 duration-500">
         <div class="p-8 border-b border-slate-100 bg-slate-50/50">
           <div class="flex items-center gap-3 mb-6">
             ...
             <div>
                <h1 class="text-lg font-black text-slate-900 uppercase tracking-tight">Document Upload</h1>
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">CharterSafe Secure Portal</p>
             </div>
           </div>

           <div class="space-y-3">
             <div class="flex justify-between items-center py-2 border-b border-slate-200/60">
                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Driver</span>
             ...
             <div class="flex justify-between items-center py-2 border-b border-slate-200/60">
                ...
                <span class="text-sm font-bold text-slate-800 bg-slate-100 px-2 py-0.5 rounded text-transform capitalize">{{ documentType }}</span>
             </div>
           </div>
         </div>
         ...
         <div class="p-8 space-y-6">
            <div class="space-y-4">
               <label class="block text-xs font-bold text-slate-700 uppercase tracking-wide">Select File to Upload</label>
               ...
               <div class="relative group">
                 ...
                 >
                   <div v-if="!selectedFile" class="flex flex-col items-center">
                     <Upload class="w-8 h-8 mb-4 opacity-50 transition-transform group-hover:-translate-y-1" />
                   ...
                   <div v-else class="flex flex-col items-center gap-2">
                     ...
                     <p class="text-sm font-black text-slate-900 text-center max-w-[200px] truncate">{{ selectedFile.name }}</p>
                     <p class="text-[10px] text-green-600 uppercase tracking-widest font-bold">Ready to upload</p>
                   </div>
                 </div>
               </div>
            </div>
...
async function upload() {
   ...
   try {
      ...
      await dataService.uploadWithToken(token, selectedFile.value)
