<template>
  <div class="bg-linear-to-br from-slate-900 to-slate-800 text-white rounded-xl shadow-lg flex flex-col h-125">
    <!-- Header -->
    ...
    <div ref="chatContainer" class="flex-1 p-4 overflow-y-auto space-y-4">
      <div v-if="chatResponse" class="bg-slate-700/50 p-4 rounded-lg text-sm leading-relaxed border border-slate-600 shadow-inner">
        <p class="font-bold text-blue-300 mb-2 flex items-center gap-2">
          <Bot :size="14" /> BusCompass AI:
        </p>
        <div class="prose prose-invert prose-sm max-w-none" v-html="chatResponse.replace(/\n/g, '<br>')"></div>
      </div>

      <div v-else class="text-center text-slate-400 text-sm mt-10 px-6">
        <div class="bg-slate-800/50 rounded-full w-12 h-12 flex items-center justify-center mx-auto mb-4 border border-slate-700">
          <Bot :size="24" />
        </div>
        <p>Ask questions about Hours of Service (HOS), Driver Qualification or Vehicle Inspections.</p>
      </div>

      <div v-if="isThinking" class="flex items-center gap-3 text-xs text-blue-300 animate-pulse bg-blue-900/20 p-2 rounded-lg border border-blue-800/30">
